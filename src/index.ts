...
/**
 * West Virginia Code — state statutes by citation.
 *
 * Section-level URLs, fully derivable: "W. Va. Code § 61-2-1" is
 * code.wvlegislature.gov/61-2-1/. No chapter file to slice, no mapping table.
 *
 * SILENT FAILURE: an unknown section returns HTTP 200 with a page of site
 * furniture and no statute (10,671 chars against 11,653 for a real one), so
 * length is nearly useless as a discriminator. Detection confirms the section
 * heading itself is rendered.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'West Virginia Code');
}

const BASE = 'https://code.wvlegislature.gov/';
const UA = 'pipeworx-mcp-west-virginia-code/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [{
  name: 'wv_code_section',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "61-2-1" (murder) or "17C-5-2" (DUI). Chapter-article-section.' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!/^\d+[A-Z]?-\d+[A-Z]?-\d+[A-Z]?$/.test(raw)) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a West Virginia Code citation.`,
      hint: 'WV sections are chapter-article-section, e.g. "61-2-1" (murder) or "17C-5-2" (DUI).' };
  }
  const url = `${BASE}${raw}/`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'section_not_found', section: raw,
      message: `No section ${raw} in the West Virginia Code (HTTP 404).` };
  }
  ...
  const start = text.search(new RegExp(`§\\s*${esc}\\.`, 'i'));
  if (start === -1) {
    return { found: false, reason: 'section_not_found', section: raw,
      message: `No section ${raw} in the West Virginia Code. The site returns a page for unknown citations, so this is read from the content rather than the status code.`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  const body = text.slice(start).trim();
  const heading = (body.match(/^§\s*[\dA-Za-z-]+\.\s*([^.]{2,120})\./) ?? [])[1] ?? null;

  return {
    citation: `W. Va. Code § ${raw}`, section: raw,
    heading: heading ? heading.trim() : null,
    text: body, text_chars: body.length, url,
    source: 'West Virginia Legislature (code.wvlegislature.gov), keyless',
    attribution: 'West Virginia statutes are public record.',
  };
}
